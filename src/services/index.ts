...
const api = {
  ...
  post: async <B, R>(url: string, body: B): Promise<R> =>
    await instance.post(url, body).then(responseBody).catch(errorHandler),
};
...
export const getProductList = (
  { page = 1, size = 10 }: ProductListReqType,
  customError = false
) => {
  return api.get<ProductListResType>(
    setQueryPath("/products", { page, size }),
    null,
    customError
  );
};
